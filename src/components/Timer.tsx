...
const DISCIPLINES = [
  { id: 'sta', name: 'Static Apnea (STA)', maxTime: 660 },
  { id: 'dyn', name: 'Dynamic Apnea (DYN)', maxTime: 600 },
  { id: 'cwt', name: 'Constant Weight (CWT)', maxTime: 300 },
  { id: 'cnf', name: 'Constant No Fins (CNF)', maxTime: 300 },
  { id: 'fim', name: 'Free Immersion (FIM)', maxTime: 300 },
];
...
export default function Timer({ onSessionComplete }: TimerProps) {
  ...
  const [phase, setPhase] = useState<'ready' | 'diving' | 'ascending' | 'surface'>('ready');
  ...
  const stopTimer = () => {
    ...
    if (time > 0) {
      ...
      onSessionComplete({
        ...
        discipline: selectedDiscipline?.name || discipline,
      });
    }
  ...
  const selectedDiscipline = DISCIPLINES.find(d => d.id === discipline);
  const progress = selectedDiscipline ? (time / selectedDiscipline.maxTime) * 100 : 0;
    ...
    <div className="space-y-6 animate-fade-in">
      <Card className="bg-ocean-deep/60 backdrop-blur-lg border-ocean-light/30 p-8">
        <div className="space-y-6">
          <div>
            <label className="text-ocean-light text-sm font-medium mb-2 block">Дисциплина</label>
            <Select value={discipline} onValueChange={setDiscipline} disabled={isRunning}>
              <SelectTrigger className="bg-ocean-mid/50 border-ocean-light/30 text-white">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="bg-ocean-deep border-ocean-light/30">
                {DISCIPLINES.map((d) => (
                  <SelectItem key={d.id} value={d.id} className="text-white hover:bg-ocean-light/20">
                    {d.name}
                  </SelectItem>
                ))}
              </SelectContent>
